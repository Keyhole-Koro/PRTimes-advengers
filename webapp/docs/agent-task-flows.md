...
フロントからは、現在の本文と prompt だけでなく、次の補助文脈も渡します。

- AI設定で明示した方針
- `固定方針` として保持する、あまり頻繁に変えない編集ルール
- 直近に `反映` または `見送り` された提案の要約と理由

これにより agent は、同じ箇所に同じ意図の提案を繰り返しにくくしつつ、文体やトーンの揺れも抑える前提で判断します。

...
const USER_AGENT = 'Mozilla/5.0 (compatible; PRTimesEditorBot/1.0; +https://example.com/link-preview)';
...
function buildPreview(html, baseUrl) {
    const title = findMetaContent(html, 'property', 'og:title') ??
        findMetaContent(html, 'name', 'twitter:title') ??
        findTitleTag(html) ??
        baseUrl.hostname;
    const description = findMetaContent(html, 'property', 'og:description') ??
        findMetaContent(html, 'name', 'description') ??
        findMetaContent(html, 'name', 'twitter:description') ??
        '';
    const imageValue = findMetaContent(html, 'property', 'og:image') ??

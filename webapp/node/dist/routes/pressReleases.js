...
pressReleaseRoutes.get('/press-releases/:id', async (c) => {
    ...
    try {
        const pool = getPool();
        const result = await pool.query('SELECT id, title, content, created_at, updated_at FROM press_releases WHERE id = $1', [id]);
...
pressReleaseRoutes.post('/press-releases/:id', async (c) => {
    ...
    try {
        ...
        await pool.query('UPDATE press_releases SET title = $1, content = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3', [title, content, id]);
        const result = await pool.query('SELECT id, title, content, created_at, updated_at FROM press_releases WHERE id = $1', [id]);
        const row = result.rows[0];
        return c.json({
            id: row.id,

...
export class PressReleaseTemplateRepository {
  async findAll(): Promise<PressReleaseTemplateRecord[]> {
    const pool = getPool()
    const result = await pool.query<PressReleaseTemplateRow>(
      `
  ...
  async findById(id: number): Promise<PressReleaseTemplateRecord | null> {
    const pool = getPool()
    const result = await pool.query<PressReleaseTemplateRow>(
      `
        SELECT id, name, title, content, created_at, updated_at
  ...
  async create(input: CreatePressReleaseTemplateInput): Promise<PressReleaseTemplateRecord> {
    const pool = getPool()
    const result = await pool.query<PressReleaseTemplateRow>(
      `
...
function mapRow(row: PressReleaseTemplateRow): PressReleaseTemplateRecord {
  return {
    id: row.id,
    name: row.name,
    title: row.title,
